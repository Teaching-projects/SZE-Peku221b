...
\subsection{Bináris mátrix}
    ...
    $$ \sum^{k\in Kertek} termeszt[1,k] =1 $$
    ...
    $$ \forall t\in Tulaj \wedge tulaj[t] =4 :\sum^{z\in Zoldsegek} termyeszt[z,t] =3 $$
...
\subsection{Where helyett implikáció}
    ...
    $$ \forall t\in Tulaj : tulaj[t]=4 \rightarrow \sum^{z\in Zoldsegek} J[z,t] =3 $$
...
\subsection{Integer mátrix}
   ...
   %kód
    ...
    $$ \forall k \in Kertek, n \in 1..Noveny : termeszt[k,n] < tulaj[k,n+1] $$
...
\subsection{Movies példa bináris mátrixszal}
    ...
    $$ \forall p \in People :  \sum^{n\in N} name[n,p] =1 $$
    $$ \forall n \in Names :  \sum^{p\in P} name[n,p] =1 $$
    ...
    $$ name[Joshua,1] + age[Joshua,db] =1 $$

    A III. típusú korlátozás két tulajdonságot kapcsol össze egymással(22. ábra).

    $$ \forall szek \in People : name[Joshua,szek] = movie[horror,szek] $$
    ...
    $$ movie[thiller,1]=0 $$
    $$ \forall szek \in 2..db : movie[thriller,szek]=1 \rightarrow shirt[black,szek]=1 $$
    $$ \forall szek\in 2..db : (movie[thriller,szek]=1) = (shirt[black,szek]=1) $$
    ...
    $$ \forall x \in People : (age[eleven,x]=1) \rightarrow (\sum^{ szek \in 1..x-1 } shirt[black,szek]=1) $$
    $$ \forall x \in People : age[eleven,x] <= \sum^{szek \in 1..x-1} shirt[black,szek] $$
   ...
   $$ \forall x,y \in People : (age[thirteen,x]=1 \wedge movie[action,y]=1) $$
   $$\rightarrow (\sum^{szek\in x+1..y-1} shirt[red,szek]=1) $$
    
   $$ \forall x \in People: movie[action,x] <= \sum^{szek\in 1..x-1} shirt[red,szek] $$
   $$ \forall x \in People: age[thirteen,x] <= \sum^{szek\in x+1..db} shirt[red,szek] $$
...
\subsection{Fundraising Dinner példa változó tömbökkel}
    ...
    $$ \forall p \in People : name[p]=Jane \leftrightarrow donation[p]=twentythousand $$
	...
	$$ \forall p \in 1..db-1 : coctail[p]=cosmopolitan \leftrightarrow age[p+1]=sixty$$ 
	$$ \wedge coctail[db] != cosmopolitan \wedge age[1] != sixty $$
    ...
    %kód
    $$ \forall a,b \in People \wedge a<b : !(coctail[a] =margarita \wedge dress[b]=blue) $$
    ...
    $$ \forall p \in 2..db-1: (name[p]=Lidia \rightarrow (coctail[p-1]=cosmopolitan \oplus coctail[p+1]=cosmopolitan)) \wedge $$
    	$$\forall p \in 2..db-1 : (coctail[p]=cosmopolitan \rightarrow (name[p-1]=Lidia \oplus name[p+1]=Lidia)) \wedge$$
    	$$(name[1]=Lidia \rightarrow coctail[2]=cosmopolitan) \wedge$$
    	$$(name[db]=Lidia \rightarrow coctail[db-1]=cosmopolitan) \wedge$$
    	$$(coctail[1]= cosmopolitan \rightarrow name[Lidia]) \wedge$$
    	$$(coctail[db]=cosmopolitan \rightarrow name[db-1]=Lidia) $$
