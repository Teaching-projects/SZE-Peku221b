...
\author{Szili Dániel, Schöffer Fruzsina, Tóth Sándor Balázs, Varga Máté\\Mérnök Informatikus BSc, I. évfolyam\\\ \\Témavezető: Dr. Hegyháti Máté\\Széchenyi István Egyetem, Informatika Tanszék}
...
\maketitle
    ...
    A logikai feladványok már a sumér társadalom emberét is foglalkoztatták, manapság viszont szerteágazó típusaival még nagyobb népszerűségnek örvendenek. Egy igen közkedvelt típus az úgynevezett zebra feladvány, mely Albert Einstein híres rejtvényéről kapta nevét. Ezek megoldása  kutatott területnek számít, több különböző matematikai és informatikai megoldás is került már publikálásra, mint például a visszalépéses keresés, korlátozási programozás vagy a feladvány Prolog nyelven való megírása.

A modern mérnöki alkalmazásokban, gépi tanulásban is egyre nagyobb népszerűségnek örvendő evolúciós algoritmusok is alkalmasak többek között ilyen jellegű kielégíthetőségi feladatok megoldására. Munkánk során egy ilyen algoritmus került kidolgozásra, és tesztelésre több példán. A futási idő csökkentése érdekében az algoritmus különböző paraméterei finomhangolásra kerültek nagy számú teszteset futtatásának tapasztalai alapján.
...
\textbf{Kulcsszavak}: logikai rejtvény, evolúciós algoritmusok, genetikus algoritmus, feladvány
...
\chapter{Bevezetés} % Máté
	...
	\noindent{Fontosabb felhasználási területei:}
...
\Abra{Einstein-pelda}{Az Einstein-példa}{width=18cm}
...
A megoldást \aref{fig_picipelda5} ábra mutatja.
...
\Abra{GA-felepites}{Az evolúciós algoritmus általános felépítése}{width=11cm}

Mint ahogyan \aref{fig_GA-felepites} ábra is mutatja, az indítást követően az első fő lépés az inicializáció, mely során feltöltjük a populációnkat egyedekkel, melyeket előállíthatunk véletlenszerűen vagy heurisztikus módszerekkel. 
...
\Abra{egypontos}{Az egypontos keresztezés}{width=10cm}

\Abra{ketpontos}{A kétpontos keresztezés}{width=10cm}
...
Ilyen mechanizmus a mutáció és a keresztezés, illetve itt kap jelentőséget az egyedek fitnesz értéke, amely gyakorlatilag a természetes kiválasztódást jelenti, így a szelekció alapját képezi. Ezek segítségével válik a populáció fejlődőképessé. 
	...
	A mutáció, keresztezés, és a mutáltak keresztezése \aref{fig_temp} ábrán látható módon egyenként \texttt{POPMERET} egyedet hoz létre, melyek egy temp nevű tömbbe kerülnek betöltésre az alappopulációval együtt. Ez mindösszesen 4-szer \texttt{POPMERET} egyedet jelent, melyek ezután a fitneszértékük alapján rendezésre kerülnek.
    A tömbből a \texttt{MEGTART} makró által megadott darabszámú egyed, vagyis a legjobb egyedek kerülnek kiválasztásra determinisztikus módon, így megvalósul az elitizmus. Ennek segítségével a biztosan jó gének nem kerülnek kizárásra a keresési térből.
    A következő generáció \texttt{POPMERET}-\texttt{MEGTART} egyede véletlenszerűen kerül kiválasztásra a rendezett egyedlista első feléből, preferálva a lista elején szereplő jobb egyedeket.
...
Shane Legg és Marcus Hutter 2005-ös értekezésükben a fitnesz függvény hatásaként létrejövő monotonitást és annak elkerülését, továbbá az egyedek diverzitásának megtartását vizsgálták\cite{Legg:2005:FUD:1068009.1068216}
        ...
        \subsection{Véletlenszerű új egyed generálás} % Dani
        ...
        \subsection{Mutálás} % Fruzsi
           		 A mutáció az egyedek paramétereit véletlenszerűen változtatja meg, így olyan egyedeket is létre tud hozni, melyek a keresztezésnél alapvetően nem biztos, hogy létrejönnének. Így képes lehet az eljárást kimozdítani egy lokális szélsőértékből és tovább tud fejlődni a 		populáció. 
   ...
   \Abra{szomszed}{A szomszédos mutáció működése egyszerűsítve}{width=10cm}
	...
	A vizsgált feladat speciális allélszerkezetéből adódóan figyelni kellett arra, hogy a csere minden esetben az azonos tulajdonsághoz tartozó értékek között történjen.
	...
	Az algoritmusban a mutáció a Megold függvényben kerül meghívásra és \texttt{POPMERET} alkalommal mutál egyedet, vagyis \texttt{POPMERET} db mutált egyed kerül a TEMP tömbbe, így negyedét kitéve annak.
        ...
        \subsection{Keresztezés} % Dani
            ...
            Figyelni kellett azonban arra, hogy csak az egyes sorok, vagyis tulajdonságok határánál vágjon a keresztező algoritmus, különben helytelen megoldást reprezentáló egyedet generált volna.
...
\begin{lstlisting}[frame=single, language=C]
	...
	Ebből kifolyólag a tesztek felépítése nem egységes, de a cél közös: ha egy adott tulajdonsággal rendelkező személy a megszabott feltételek alapján a megfelelő pozícióban van (mi esetünkben: széken ül), akkor a függvény (Teszt) 0-át ad vissza értékül, különben pedig 1-et.
	...
	\Forraskod{tesztek.c}{frame=single , lastline=19}
...
\chapter{Empírikus vizsgálatok} % Máté
    ...
    A példa pontos adatai \aref{fundraiser} függelékben találhatók.
	...
	A program futásakor mindig az aktuális generáció legjobb egyedét mutatja meg, valamint azt, hogy az hány megkötést sért meg, illetve pontosan melyikeket, ahogy az  \aref{fig_test-in-progress} ábrán látható.

	\Abra{test-in-progress}{A program a futás kezdetekor}{width=14cm}
	...
	\Aref{fig_results} ábra mutatja be, hogy miként változik meg a futási sebesség, valamint az megoldás megtalálásához szükséges generációk száma a populáció és a megtartott egyedek számának különböző értékei mellett.
	...
	\Abra{results}{Futási eredmények különböző paraméterek esetén}{width=13cm}
...
\chapter{Összefoglalás és jövőbeni tervek} % Máté
...
\section*{Tulajdonságok és értékeik}
	\begin{description}
	\item [Dress] [ Black | Blue | Purple | Red | White ]
	\item [Name] [ Carmen | Diana| Jane | Lidia | Melissa ]
	\item [Donation] [ 10.000 dollars | 20.000 dollars | 30.000 dollars | 40.000 dollars | 50.000 dollars ]
	\item [Coctail] [ Cosmopolitan | Daiquiri | Manhattan | Margarita | Martini ]
	\item [Necklace] [ Emerald | Moonstone | Pearl | Sapphire | Turquoise ]
	\item [Age] [ 40 | 45 | 50 | 55 | 60 ]
...
\section*{Megkötések}
	...
	\begin{itemize}
	\item The woman who donated 30,000 dollars is immediately before the youngest woman. 
	\item Diana is exactly to the left of the 50 years old woman. 
	\item The donator wearing the Emerald necklace is exactly to the left of the donator wearing the Purple dress. 
	\item The lady wearing the Pearl necklace donated the smallest amount. 
	\item The guest drinking Martini is next to the guest who donated 10,000 dollars. 
	\item The woman wearing the Turquoise necklace donated 30,000 dollars. 
	\item The oldest woman is exactly to the right of the lady drinking Cosmopolitan. 
	\item The donator wearing the White dress is next to the woman who gave the biggest donation. 
	\item The woman wearing the Red dress is somewhere between the woman that donated $ 20,000 and the woman that gave $ 40,000, in that order. 
	\item Lidia is next to the guest wearing the Black dress. 
	\item Jane donated 20,000 dollars. 
	\item Melissa is exactly to the right of the guest drinking Cosmopolitan. 
	\item The lady wearing the Blue dress is somewhere to the left of the lady drinking Margarita. 
	\item The guest that donated 20,000 dollars is immediately before the guest drinking Daiquiri. 
	\item The 50 years old woman is exactly to the right of the woman wearing the Pearl necklace. 
	\item Lidia is next to the woman drinking Cosmopolitan. 
	\item The lady wearing the Black dress is somewhere to the left of the lady who donated 40,000 dollars. 
	\item The 55 years old woman is wearing the Sapphire necklace. 
	\item The 60 years old woman is somewhere to the right of the woman wearing the Black dress. 
	\item The lady drinking Manhattan is exactly to the right of the lady wearing the Red dress. 
	\item The guest wearing the White dress is somewhere to the left of the guest wearing the Black dress.
	\end{itemize}
